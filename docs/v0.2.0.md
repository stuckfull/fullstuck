...
- **Shorthand Text**: `"h1" => '$var'` (Menimpa text konten, kebal XSS)
- **Atribut**: `"[src]" => '$imgUrl'`
- **`@foreach`**: `"li" => ["@foreach" => '$items as $i']` (Looping elemen target)
- **`@if`**: `"div.promo" => ["@if" => '$isPromo']` (Show/Hide)
- **`@html`**: `"div" => ["@html" => '$rawHtml']` (Injeksi WYSIWYG, awas celah XSS)
- **`@append` / `@prepend`**: `"head" => ["@append" => '"<style>..."']` (Injeksi raw HTML secara non-destruktif)
- **`@remove`**: `"[style]" => "@remove"` (Hapus atribut HTML statis)
