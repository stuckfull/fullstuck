...
2. **AI Cheatsheet (API Hemat Token)**: [cheatsheet.md](cheatsheet.md) (`https://raw.githubusercontent.com/stuckfull/fullstuck/main/docs/v0.2.0_cheatsheet.md`)
...
- **`spa`**: Konfigurasi mode *Single Page Application* (SPA) bawaan:
  ...
  - `history_cache`: Mengontrol perilaku tombol Back/Forward browser. `false` (default) selalu melakukan *re-fetch* ulang dari server agar middleware dan keamanan tetap tereksekusi. `true` akan me-replay cache HTML lama (seketika, tapi bisa *stale*).
...
File HTML dijaga 100% statis tanpa tag PHP (`<?= ?>` / <code v-pre>{{ }}</code>). Logic injeksi diurus via array deklaratif di PHP. **Zero XSS by Default**.
...
*   **`data-fst-cache="true|false"`**
    Mengontrol cache saat tombol Back/Forward browser ditekan, khusus untuk link ini. Menimpa konfigurasi global `history_cache`.
    ```html
    <a href="/tentang" data-fst-cache="true">Tentang (Instant Cache)</a>
    <a href="/dasbor" data-fst-cache="false">Dasbor (Selalu Fresh)</a>
    ```
...
2. **Back & Forward (Scroll Restoration)**: Secara default, navigasi *history* akan melakukan *fetch* ulang ke server demi keamanan (memastikan middleware/auth tetap dijalankan) dan mengembalikan posisi scroll ke tempat terakhir pengguna berada. Jika Anda ingin pemuatan seketika dari cache (tanpa *fetch*), gunakan opsi konfigurasi `history_cache: true` atau atribut `data-fst-cache="true"`.
...
*   **`fst.go(url, options)`**
    ...
    *   `options` (object, opsional):
        ...
        *   `cache` (boolean): Simpan halaman ini di cache untuk pemuatan seketika saat Back/Forward.
